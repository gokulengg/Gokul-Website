...
interface QuoteModalProps {
  ...
  renderTrigger?: boolean; // whether to render the default trigger button (defaults to true)
}

const QuoteModal: React.FC<QuoteModalProps> = ({ onOpenChange, open: openProp, renderTrigger = true }) => {
  ...
  const effectiveOpen = controlled ? openProp! : openState;

  const handleOpenChange = (val: boolean) => {
    if (!controlled) setOpenState(val);
      ...
      <DialogContent className="max-w-[92%] sm:max-w-3xl max-h-[90vh] flex flex-col p-0 rounded-lg">
        ...
        >
          ...
          <div className="md:col-span-2">
            {/* inline status below inputs for small screens (optional) */}
